...
class CanvasClient:
    ...
    async def get_client(self) -> aiohttp.ClientSession:
        ...
        if self.client is None or self.client.closed:
            self.client = aiohttp.ClientSession(
                headers=self.headers, timeout=aiohttp.ClientTimeout(total=30.0), trust_env=True
            )
        return self.client
    ...
    async def get_assignment_submission(
        ...
        """Fetch the current user's submission for a specific assignment."""
        path = f"/api/v1/courses/{course_id}/assignments/{assignment_id}/submissions/self"
        res = await self._get(path)
        return await res.json()

    async def is_assignment_submitted(
        self, course_id: int, assignment_id: int
    ) -> tuple[bool, str]:
        """
        Check if an assignment has been submitted.
        Returns: (is_submitted, status_string)
        """
        try:
            submission = await self.get_assignment_submission(course_id, assignment_id)
            workflow_state = submission.get("workflow_state", "unsubmitted")
            # States: submitted, graded, pending_review, unsubmitted, etc.
            is_submitted = workflow_state in ("submitted", "graded", "pending_review")
            return is_submitted, workflow_state
        except Exception:
    ...
    async def submit_assignment(
        ...
        for file_path in file_paths:
            ...
            with open(file_path, "rb") as f:
                ...
                async with aiohttp.ClientSession(trust_env=True) as upload_session:
